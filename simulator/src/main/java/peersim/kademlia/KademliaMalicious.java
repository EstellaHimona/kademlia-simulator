...
public class KademliaMalicious extends KademliaProtocol {
   ...
   */
  public Object clone() {
    KademliaMalicious dolly = new KademliaMalicious(KademliaMalicious.prefix);
    return dolly;
  }
   ...
   * @param prefix String
   */
  public KademliaMalicious(String prefix) {
    super(prefix);
    this.node = null; // empty nodeId
    KademliaMalicious.prefix = prefix;
   ...
   * KademliaMalicious. Every node shares the same configuration, so it is sufficient to call this
   * routine once.
   */
  private void _init() {
    // execute once
  ...
  private void handleInit(Message m, int myPid) {
    ...
    if (m.type == Message.MSG_FIND_DIST) {
      System.out.println("Message received (FIND_DIST): " + m + "!!!");
    }

    if (m.type == Message.MSG_INIT_GET) {
      System.out.println("Message received (INIT_GET): " + m);
    }
  }
  ...
  public void sendMessage(Message m, BigInteger destId, int myPid) {
    ...
    // destpid = dest.getKademliaMalicious().getProtocolID();
  ...
  public void processEvent(Node myNode, int myPid, Object event) {
    ...
    switch (((SimpleEvent) event).getType()) {
      ...
      case Message.MSG_INIT_GET:
        m = (Message) event;
        System.out.println("MSG_INIT_GET received");
        break;
